...
if draft_result.success?
  ...
  if draft_result.respond_to?(:errors) && draft_result.errors.respond_to?(:full_messages)
    puts "    Errors: #{draft_result.errors.full_messages}"
  end
end
...
if ended_result.success?
  ...
  if ended_result.respond_to?(:errors) && ended_result.errors.respond_to?(:full_messages)
    puts "    Errors: #{ended_result.errors.full_messages}"
  end
end
...
if draft_topics.empty?
  ...
  draft_topics.each do |t|
    puts "  - #{t.title} (user: #{t.user.username}, created: #{t.created_at})"
  end
end
puts ""
